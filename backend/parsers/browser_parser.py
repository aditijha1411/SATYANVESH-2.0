...
BROWSER_ALIASES = {
    "phone_number": ["phone_number", "device_id", "user", "msisdn"],
    "url": ["url", "website", "domain", "visited_url", "page_url"],
    "title": ["title", "page_title", "site_name"],
    "timestamp": ["timestamp", "visit_time", "date_time", "datetime", "accessed_at"],
    "duration": ["duration", "time_spent", "visit_duration", "dwell_time"]
}
...
class BrowserParser(BaseParser):
        ...
        try:
            if ext == ".json":
                with open(self.file_path, "r") as f:
                    data = json.load(f)
                records = data.get("browser_history", data) if isinstance(data, dict) else data
    ...
    def _map_columns(self):
        for standard, aliases in BROWSER_ALIASES.items():
    ...
    def extract_entities(self, records: List[Dict]) -> List[Dict]:
        entities = {}
        for r in records:
            for field, etype in [("url", "url"), ("phone_number", "phone_number")]:
