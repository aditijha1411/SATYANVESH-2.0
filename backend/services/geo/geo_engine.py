...
class GeoEngine:
        ...
        for event in sorted_events:
            if event.location:
                routes.append({
                    ...
                    "latitude_longitude": event.target_entity
                })
        return routes
        ...
        for e1 in events1:
            for e2 in events2:
                if e1.location == e2.location:
                    time_diff = abs(int(e1.timestamp[11:13]) - int(e2.timestamp[11:13])) if e1.timestamp and e2.timestamp else 0
                    if time_diff <= 1:
                        meetings.append({
                            ...
                            "confidence": 0.85 if time_diff == 0 else 0.65
                        })
        return meetings
    ...
    def geofence_analysis(self, entity_value: str, geofence_location: str, radius_km: float = 1.0) -> List[Dict]:
        events = self.db.query(Event).filter(
            Event.case_id == self.case_id,
            Event.event_type == "GPS_MOVEMENT",
            Event.source_entity == entity_value
